...
void tnp_PlotEff_ISO( bool isSave = true ) {
...
////////////////////////////////
  for (int ipt=0; ipt<BinPtSize-1; ipt++){
     ...
     TCanvas* tDATA = (TCanvas*) DATA->Get(Form("tpTree/Medium_ISO_ptVSeta/fit_eff_plots/eta_PLOT_pt_bin%d_&_Medium_pass%s",ipt,Tag_trig.Data() ));
     ...
     TCanvas* tDATA_ptGt20 = (TCanvas*) DATA->Get(Form("tpTree/Medium_ISO_ptVSeta_ptGt20/fit_eff_plots/eta_PLOT_pt_bin%d_&_Medium_pass%s",ipt,Tag_trig.Data() ));
     TGraphAsymmErrors* grDATA_ptGt20 = (TGraphAsymmErrors*) tDATA_ptGt20 -> GetListOfPrimitives()->At(1);
     ErrorCheck(grDATA_ptGt20);

     TCanvas* tDATA_ptLt20 = (TCanvas*) DATA->Get(Form("tpTree/Medium_ISO_ptVSeta_ptLt20/fit_eff_plots/eta_PLOT_pt_bin%d_&_Medium_pass%s",ipt,Tag_trig.Data() ));
     ...
     ErrorCheck(grDATA_ptLt20);
         ...
         else if (sample_data == "TnP_ISOTight_Run2016B_PTvsETA"){
             PicName = "ISO_Run2016B_PTvsETA";
             tl->AddEntry(grDATA, "Run 2016B, run #geq 273423"                          ,"lp");
             tl->AddEntry(grDATA, "PF ISO < 0.15 eff."     ,"");
         }
